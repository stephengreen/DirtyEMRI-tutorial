...
class ModuleTest(unittest.TestCase):
    ...
    def test_trajectory_KerrEccentricEquatorial(self):
        ...
        np.random.seed(19960126)
        
        # run trajectory
        for ii in range(100):
            # print(ii)
            mu = 10**np.random.uniform(0.0, 3)
            M = 10**np.random.uniform(5, 7)
            a = np.random.uniform(0.01, 0.99)
            p0 = np.random.uniform(8.0, 20.0)
            t, p, e, x, Phi_phi, Phi_theta, Phi_r = traj(M, mu, a, p0, e0, 1.0, 0.0, 8.0, 0.0, T=4.0, dt=10.0)

        # t, p, e, x, Phi_phi, Phi_theta, Phi_r = traj(M, mu, a, p0, e0, 1.0, 0.0, 3.0, 3.0, **insp_kw)