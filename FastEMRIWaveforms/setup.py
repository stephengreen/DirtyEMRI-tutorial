...
setup(
    ...
    # ship the runtime data tables so an `install` is self-contained (tutorial patch)
    package_data={"few": ["files/*.dat", "files/*.hdf5"]},
    classifiers=[
...
# --- tutorial patch ---------------------------------------------------------
# Post-install: copy the runtime data tables next to the installed `few` package.
# The original code ran this unconditionally and assumed an egg layout, which
# crashes under `build_ext --inplace`, `develop`, and modern pip installs.
# We now (a) only run it for an actual `install`, and (b) never let it abort the
# build. With `package_data` set above, a normal install already ships the data,
# so this is just a best-effort fallback.
if "install" in sys.argv:
    try:
        import importlib, glob, site
        importlib.import_module("few")
        location = site.getsitepackages()[0]
        matches = glob.glob(location + "/few*/few/") or glob.glob(location + "/few/")
        if matches:
            folder_path = matches[0] + "files"
            os.makedirs(folder_path, exist_ok=True)
            os.system("cp -r ./few/files/* " + folder_path + "/")
            print("Transferred data files to " + folder_path)
    except Exception as e:  # never break the build over the data copy
        print("Skipping post-install data copy ({}). Data files remain in the "
              "source tree at ./few/files/.".format(e))